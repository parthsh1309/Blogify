...
function Blogs({ filterVisible, setFilterVisible }) {
  const [blogs, setBlogs] = useState([]);

  useEffect(() => {
    blogService
      .getBlogs(false, "All", 10)
      .then((res) => {
  ...
  return (
    <div className="sm:w-3/4">
      <div className="flex justify-center items-center gap-2">
        <button className="sm:hidden text-lg text-white text-center border border-gray-700 w-fit m-auto py-2 px-4 rounded-full font-merri bg-gray-900 flex items-center gap-2" onClick={() => setFilterVisible(!filterVisible)}>
